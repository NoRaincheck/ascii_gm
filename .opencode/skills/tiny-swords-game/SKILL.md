...
- Water foam: `GameScene.buildFoam()` places an animated `foam` sprite (192×192, a 3×3 grid of 64px tiles) centered on every land tile where `landTouchesWater(world, tx, ty)` is true (beach/grass/rock with an orthogonal sea neighbor). The foam blob is centered under the opaque land tile (depth −8, below beach −7/grass −6/rock), so the full foam center is hidden and only the outer foam strips show as ripples over the adjacent water. The foam sprites are masked with a stencil mask (`Graphics.fillRect` covering the full map area with padding, `createGeometryMask()`, graphics kept off the display list with `add: false`): shore land tiles have transparent edge speckles in their art, and without the mask the blob body bleeds through them as white flecks on grass/beach/rock — including on edges facing the cliff band. Start frames are staggered (`(i * 3) % 8`) so adjacent foam isn't animated in lockstep.
